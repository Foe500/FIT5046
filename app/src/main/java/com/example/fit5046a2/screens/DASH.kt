...
private fun ContextAlert(riskLevel: String) {
    if (riskLevel == "High Risk") {
        Card(
            modifier = Modifier.fillMaxWidth(),
            colors = CardDefaults.cardColors(containerColor = Color.Yellow.copy(alpha = 0.2))
        ) {
...
private fun HealthOverview() {
    ...
    Card(
        modifier = Modifier.fillMaxWidth(),
        colors = CardDefaults.cardColors(containerColor = Color.Purple.copy(alpha = 0.1))
    ) {
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            LinearProgressIndicator(
                progress = normalCount.toDouble() / (normalCount + abnormalCount),
